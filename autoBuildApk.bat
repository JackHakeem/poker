...
mkdir src_temp
del res\project.manifest
del res\version.manifest
call encrypt_res.bat -i res -o res_temp -ek woyaopoker -es poker
rem XCOPY res res_temp\ /s /h /d /y
if exist res_temp\shareicon (
	rmdir res_temp\shareicon /s /q
	XCOPY res\shareicon res_temp\shareicon\ /s /h /d /y
)

for %%i in (.\res_temp\audio\*.mp3) do del %%i
...
COPY /y res_temp\project.manifest res\ 
COPY /y res_temp\version.manifest res\ 
