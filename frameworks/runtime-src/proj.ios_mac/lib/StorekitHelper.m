...
@implementation StorekitHelper{
    ...
    -(void)buyProduct:(NSDictionary *) params{
        if([SKPaymentQueue canMakePayments]) {
            ...
            mPid = [[NSString alloc] initWithString:[params objectForKey:@"pid"]];
            mOrderid = [[NSString alloc] initWithString:[params objectForKey:@"orderid"]];
            mPmode = [[NSString alloc] initWithString:[params objectForKey:@"pmode"]];
            NSData *data =  [NSJSONSerialization dataWithJSONObject:params options:kNilOptions error:nil];
            applicationUsername = [[NSString alloc]initWithData:data encoding:NSUTF8StringEncoding];
            [self requestProductData:mProductID];
        }else{
            NSLog(@"不允許程序內付費購買");
            UIAlertView *alerView = [[UIAlertView alloc]initWithTitle:NSLocalizedString(@"提示",nil) message:NSLocalizedString(@"您的手機沒有打開程序內付費購買",nil) delegate:nil cancelButtonTitle:NSLocalizedString(@"關閉",nil) otherButtonTitles:nil];
            [alerView show];
        }
    }
    ...
    -(void)requestProductData:(NSString *) productID{
        ...
        [SVProgressHUD showWithStatus:NSLocalizedString(@"正在請求商品信息",nil) ];
    ...
    -(void)productsRequest:(SKProductsRequest *)request didReceiveResponse:(SKProductsResponse *)response{
        ...
        if(myProduct.count == 0) {
            ...
            [SVProgressHUD showErrorWithStatus:NSLocalizedString(@"交易失敗，請重試",nil)];
            return;
        }
        ...
        NSLog(@"---------发送购买请求------------");
        [SVProgressHUD setDefaultMaskType:SVProgressHUDMaskTypeBlack];
        [SVProgressHUD showWithStatus:NSLocalizedString(@"正在發送購買請求",nil)];
...
//        [SVProgressHUD showErrorWithStatus:[error localizedDescription] maskType:SVProgressHUDMaskTypeBlack];
        ...
        for (SKPaymentTransaction *transaction in transactions)
        {
            switch (transaction.transactionState)
            {
                case SKPaymentTransactionStatePurchased://交易完成
                    [SVProgressHUD setDefaultMaskType:SVProgressHUDMaskTypeNone];
                    [SVProgressHUD showSuccessWithStatus:NSLocalizedString(@"交易完成",nil)];
                    NSLog(@"transactionIdentifier = %@", transaction.transactionIdentifier);
                    NSLog(@"applicationUsername = %@", transaction.payment.applicationUsername);
                ...
                case SKPaymentTransactionStateRestored://已经购买过该商品
                    [SVProgressHUD setDefaultMaskType:SVProgressHUDMaskTypeNone];
                    [SVProgressHUD showErrorWithStatus:NSLocalizedString(@"已經購買過商品",nil)];
                    [self restoreTransaction:transaction];
                    break;
                case SKPaymentTransactionStatePurchasing:      //商品添加进列表
                    NSLog(@"商品添加进列表");
                    [SVProgressHUD setDefaultMaskType:SVProgressHUDMaskTypeBlack];
                    [SVProgressHUD showWithStatus:NSLocalizedString(@"正在請求付費信息",nil) ];
                    break;
                default:
                    [SVProgressHUD dismiss];
                    break;
    ...
    - (void)completeTransaction:(SKPaymentTransaction *)transaction {
        ...
        NSError *err;
        NSDictionary *params = [NSJSONSerialization JSONObjectWithData:jsonData options:NSJSONReadingMutableLeaves error:&err];
        NSString * productID;
        NSString * pid = [[NSString alloc] initWithString:[params objectForKey:@"pid"]];
        NSString * orderid = [[NSString alloc] initWithString:[params objectForKey:@"orderid"]];
        NSString * pmode = [[NSString alloc] initWithString:[params objectForKey:@"pmode"]];
        if(err){
            ...
            productID = [[NSString alloc] initWithString:[params objectForKey:@"productID"]];
    ...
    - (void)failedTransaction:(SKPaymentTransaction *)transaction {
        if(transaction.error.code != SKErrorPaymentCancelled) {
            ...
            NSLog(@"用户取消交易");
            NSMutableDictionary *dict = [[[NSMutableDictionary alloc] init] autorelease];
            [dict setValue:mPmode forKey:@"pmode"];
