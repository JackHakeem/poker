...
function GameSocket:createSocket()
	...
	self.socket_:setConnFailTime(6)
end
...
function GameSocket:connect( host,port )
	...
	if tt.owner:getUid() == 0 then return end
...
function GameSocket:onData( evt )
	...
	do
		...
		if size < pack_len + PKG_HEAD_LEN then
		...
		local data = self.buff_:sub(1,pack_len + PKG_HEAD_LEN)
		self.buff_ = self.buff_:sub(pack_len + PKG_HEAD_LEN + 1)
