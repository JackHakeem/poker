...
local TouzhuItem = class("TouzhuItem", function()
	return display.newNode()
end)


function TouzhuItem:ctor(control)
	self.control_ = control 
	--find view by layout
	local node, width, height = cc.uiloader:load("touzhu_item.json")
	self:addChild(node)
	self.root_ = node
	...
	self:addNodeEventListener(cc.NODE_TOUCH_EVENT, function(event)
	    ...
	    elseif event.name ~= "began" and down then
	    	...
	    	if event.name == "ended" then
				...
				self.control_:showTouzhuGameDialog()
			end
		end
	end)
end
