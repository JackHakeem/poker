...
local TouzhuGameRoomView = class("TouzhuGameRoomView", function(...)
	return BLDialog.new(...)
end)

function TouzhuGameRoomView:ctor(control)
	self.control_ = control 
	local node, width, height = cc.uiloader:load("touzhugame_room_view.json")
	self:addChild(node)
	self.root_ = node
	...
	self.mMyActions = {}
	self.mSelectCoins = 0
	self.mMyActionBg = cc.uiloader:seekNodeByName(node,"my_action_bg")
	self.mMyActionBg:setPositionX(-146)
...
function TouzhuGameRoomView:addTouchEvent()
	self:setTouchEnabled(true)
		-- 注册触摸事件
	self:addNodeEventListener(cc.NODE_TOUCH_EVENT, function(event)
	    local x,y = event.x,event.y
	    if event.name == "began" then
	    	if not ( self.mContentBg:getCascadeBoundingBox():containsPoint(cc.p(x, y)) or self.mMyActionBg:getCascadeBoundingBox():containsPoint(cc.p(x, y)) )  then
	    		self:dismiss()
	    	end
	    end	
	end)
...
function TouzhuGameRoomView:showLoad()
...
function TouzhuGameRoomView:show(str)
	...
	self.mContentBg:setPositionX(359+1280)
	self.mContentBg:moveTo(0.2, -359+1280)
end

function TouzhuGameRoomView:dismiss()
	...
	self.mContentBg:setPositionX(-359+1280)
	self.mContentBg:moveTo(0.2, 359+1280)
	self.mContentBg:performWithDelay(function()
			self:removeSelf()
		end, 0.2)
end

function TouzhuGameRoomView:initView()
	self.mIsLoaded = true
...
function TouzhuGameRoomView:subNum(num)
...
function TouzhuGameRoomView:selectCoin(index)
...
function TouzhuGameRoomView:updateCoinsBtnsStatus()
	local coins = tt.owner:getCoins() - self.mSelectCoins
...
function TouzhuGameRoomView:checkPreSelectCoins()
...
function TouzhuGameRoomView:checkShowRecommendDialog()
...
function TouzhuGameRoomView:addAction(num,coins)
	if num < 0 then
		coins = coins * 8
	end
	...
	if self.mMyActions[num] then
		...
		local content = app:createView("TouzhuActionRoomItem", self)
		local size = cc.size(288,84)
...
function TouzhuGameRoomView:subAction(num,coins)
	if num < 0 then
		coins = coins * 8
	end
...
function TouzhuGameRoomView:clearAllAction()
...
function TouzhuGameRoomView:updateActionView()
	self.mMyActionBg:stopAllActions()
	if not next(self.mMyActions) then
		self.mMyActionBg:moveTo(0.2, -146)
	else
		self.mMyActionBg:moveTo(0.2, 146)
	end
end

function TouzhuGameRoomView:playBetAnim()
	for index,num in ipairs(self.mNumContents) do
		if self.mMyActions[num] then
...
function TouzhuGameRoomView:updateForecastView()
	local minCoins
	local maxCoins 
	...
	self.mStarIcon:setPositionX(245/2-size.width/2-15)
end

function TouzhuGameRoomView:setSelectBtnNum(btn,isNor,num)
	if num >= 3 and num <= 18 then
...
function TouzhuGameRoomView:updateNumSelectBtnsFactors()
	for index,num in ipairs(self.mNumContents) do
		if self.mMyActions[num] then
			local btn = self.mNumSelectBtns[index]
...
function TouzhuGameRoomView:updateNumSelectBtns()
	for index,num in ipairs(self.mNumContents) do
		if self.mMyActions[num] then
			local btn = self.mNumSelectBtns[index]
			if num >= 3 and num <= 18 then
...
function TouzhuGameRoomView:onBet()
	if self.mBeting then return end
	...
	if next(self.mMyActions) then
		...
		if next(bets) then
			self:showLoad()
			self.mBeting = true
			tt.gsocket.request("happydice.bet",{mid=tt.owner:getUid(),place="room",bets=bets})
		end
	end
end


function TouzhuGameRoomView:onPayBtnPressed()
	if not tolua.isnull(self.mPayBtnAnimNode) then
		self.mPayBtnAnimNode:setPositionY(62)
	end
end


function TouzhuGameRoomView:onPayBtnRelease()
	if not tolua.isnull(self.mPayBtnAnimNode) then
		self.mPayBtnAnimNode:setPositionY(78)
	end
end

function TouzhuGameRoomView:updatePayBtnStatus()
...
function TouzhuGameRoomView:setCurStage(happyid,stage)
...
function TouzhuGameRoomView:startCurStageCountDown(time)
...
function TouzhuGameRoomView:setLastStage(happyid,stage)
...
function TouzhuGameRoomView:setLastLuck(luck)
	if next(luck) then
		for i=1,3 do
			local num = luck[i]
...
function TouzhuGameRoomView:playLuckAnim(luck)
...
function TouzhuGameRoomView:addHistoryLotteryItems(data)
	local item = self.mHistoryLotteryList:newItem()
	local content = app:createView("TouzhuLuckRoomItems", self.control_)
...
function TouzhuGameRoomView:onHistoryLotteryLoad(datas)
...
function TouzhuGameRoomView:onExit()
...
function TouzhuGameRoomView:onSocketData(evt)
	print("TouzhuGameRoomView onSocketData",evt.cmd)
